...
static TASK *alloc_task_struct(int stack_size)
{
        ...
        {
                ss = (short *)p;
                j = (sizeof(TASK) / sizeof(short)) + (stack_size * sizeof(int)) / sizeof(short);
                for (i = 0; i < j; i++)
                        ss[i] = STACKFREE;
        }
...
static void initialize_task_struct(TASK *p, char *name, void (*func)(ASIZE arg), ASIZE arg, int stacksize)
{
        p->name = name;
        p->func = func;
        p->arg = arg;
...
void insert_task_link(TASK *p)
{
        DISABLE_TIMERS;
        if (current == 0)
        { /* initialize current to first task created */
                current = p;
...
void delete_task_link(TASK *p)
{
        ...
        DISABLE_TIMERS;
        if (t == p)
                return;    /* don't kill last task. */
        t->next = p->next; /* link around p */

        if (p == current)
        { /* did we link around calling task? */
                current = t->next;
                ENABLE_TIMERS;
                free(p);
...
*/
        ...
        if ((p = alloc_task_struct(stacksize)) == (TASK *)-1)
                return -1;
